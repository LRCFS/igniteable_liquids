...
# neat
neat.dat = data.frame(row.names = RTnames, R1 = dat$AREA1..5, R2 = dat$AREA2..7, R3 = dat$AREA3..9)
neat.m = mungeData(neat.dat, name='Neat')

# 10% weathered
p10.dat = data.frame(row.names = RTnames, dat[,13], dat[,15], dat[,17])
p10.m = mungeData(p10.dat, name='10pc')

# 25% weathered
p25.dat = data.frame(row.names = RTnames, dat[,20], dat[,22], dat[,24])
p25.m = mungeData(p25.dat, name='25pc')

# 50% weathered
p50.dat = data.frame(row.names = RTnames, dat[,27], dat[,29], dat[,31])
p50.m = mungeData(p50.dat, name='50pc')

# 75% weathered
p75.dat = data.frame(row.names = RTnames, dat[,34], dat[,36], dat[,38])
p75.m = mungeData(p75.dat, name='75pc')

# 90% weathered
p90.dat = data.frame(row.names = RTnames, dat[,41], dat[43], dat[,45])
p90.m = mungeData(p90.dat, name='90pc')

# 95% weathered
p95.dat = data.frame(row.names = RTnames, dat[,48], dat[,50], dat[,52])
...
#all.dat = rbind(neat.m, p10.m, p25.m)
